...
def get_key_data(po, pkghead, enc_k_fourcc):
    ...
    for k in key_list:
        if k in keys.keys():
            enc_key.append(keys[k])

    return enc_key

def choose_enc_key(po, key_list):
    enc_key = key_list[0]
    return enc_key
...
def imah_get_crypto_params(po, pkghead):
    ...
    if enc_k_str != '':
        enc_keys = get_key_data(po, pkghead, enc_k_str)
        enc_key = enc_keys[0]
        print(enc_key)
    else:
        enc_key = bytes()
...
def imah_get_auth_params(po, pkghead):
    ...
    for key_data in auth_key_data:
        if key_data is None:
            eprint("{}: Warning: Cannot find auth_key '{:s}'".format(po.sigfile,auth_k_str))
            return (None)
        if isinstance(key_data, str):
            auth_key = RSA.importKey(key_data)
        elif len(key_data) == sizeof(ImgRSAPublicKey):
            auth_key_struct = ImgRSAPublicKey()
            memmove(addressof(auth_key_struct), key_data, sizeof(auth_key_struct))
            ...
            return (None)
        auth_key_list.append(auth_key)
    return (auth_key_list)
...
def imah_unsign(po, fwsigfile):
    ...
    auth_keys = imah_get_auth_params(po, pkghead)
...
def tar_decompress(po):
    tar = tarfile.open(po.tarfile)
    output_dir = "./decfiles"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    tar.extractall(path=output_dir)
    sig_files = tar.getnames()
    tar.close()
    os.chdir(output_dir)
    return sig_files
...
def decompress(po):
    if (po.verbose > 0):
        print("{}: Opening for decompressing".format(po.tarfile))
    sig_files = tar_decompress(po)
    if (po.verbose > 2):
        print("Total file list: ", sig_files)
    
    for file in sig_files:
        po.sigfile = file
        po.mdprefix = os.path.splitext(os.path.basename(po.sigfile))[0]
        po.unsign = True
        print("[*] Unsigning file: ", po.sigfile)
        if (po.verbose > 2):
            print(po)
        unsign(po)
...
def main():
    ...
    parser.add_argument('-t', '--tarfile', default="", type=str,
          help=("directory and file name of signed and encrypted (tar) firmware file"))
    ...
    subparser.add_argument('-d', '--decompress', action="store_true",
          help=("decompress tar firmware file"))
